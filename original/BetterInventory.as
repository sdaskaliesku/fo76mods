...
package
{
   ...
   public class BetterInventory extends Sprite
   {
      ...
      public function ProcessUserEvent(param1:String, param2:Boolean) : Boolean
      {
         ...
         if(this.shiftKeyDown && !param2)
         {
            switch(param1)
            {
               ...
               case "StrafeLeft":
               case "Left":
                  this.pipboyMenu.TryToSetTab(Math.max(TAB_WEAPONS_INDEX,this.pipboyMenu.DataObj.CurrentTab - 2));
                  return true;
               case "StrafeRight":
               case "Right":
                  this.pipboyMenu.TryToSetTab(Math.min(TAB_AMMO_INDEX,this.pipboyMenu.DataObj.CurrentTab + 2));
                  return true;
            }
         }
         ...
         while(_loc4_ < param1.length)
         {
            ...
            if(_loc5_.filterFlag & param2)
            {
               if(this.itemInfoMap[_loc5_.serverHandleID] == null)
               {
                  _loc6_ = [];
                  _loc7_ = new Array();
                  this.invPage.codeObj.onInvItemSelection(_loc4_,_loc6_,_loc7_,this.pipboyMenu,_loc5_.serverHandleID);
      ...
      private function redraw() : void
      {
         ...
         var weightStr:String = Math.floor(this.pipboyMenu.DataObj.CurrWeight) + "/" + Math.floor(this.pipboyMenu.DataObj.MaxWeight) + " [" + this.currentTabWeight + "]";
                     ...
                     if(getQualifiedClassName(itemCard) == "ItemCard_StandardEntry")
                     {
                        if(itemCard.Label_tf.text == this._weightStr)
                        {
                           ...
                           if(labelWidthDiff > 0)
                           {
                              ...
                              itemCard.Value_tf.width = itemCard.Value_tf.width + labelWidthDiff;
                           }
                           ...
                           itemCard.Value_tf.text = weightSingle + " [" + totalWeight.toFixed(weightSingleDecimalPlaces) + "]";
                           break;
                        }
                     }
                     i++;
      ...
      private function RoundDecimal(param1:Number, param2:uint) : String
      {
         ...
         if(_loc4_ > -1)
         {
            ...
            while(_loc5_ >= _loc4_)
            {
               ...
               {
                  return _loc3_.substring(0,_loc5_ == _loc4_?Number(_loc5_):Number(_loc5_ + 1));
               }
               _loc5_--;
            }
         }
