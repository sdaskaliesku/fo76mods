...
package fl.motion
{
   ...
   public class Animator3D extends AnimatorBase
   {
      ...
      private static var IDENTITY_MATRIX:Matrix = // method body index: 688 method index: 688
      new Matrix();
      
      protected static const EPSILON:Number = // method body index: 688 method index: 688
      1.0e-8;
      ...
      override protected function setTime3D(param1:int, param2:MotionBase) : Boolean
      {
         ...
         var _loc13_:Matrix3D = param2.getMatrix3D(param1) as Matrix3D;
         ...
         if(_loc13_)
         {
            if(!motionArray || !_lastMatrix3DApplied || !matrices3DEqual(_loc13_,Matrix3D(_lastMatrix3DApplied)))
            {
               _loc3_ = _loc13_.clone();
               if(this._initialMatrixOfTarget)
               {
                  _loc3_.append(this._initialMatrixOfTarget);
               }
               this._target.transform.matrix3D = _loc3_;
               _lastMatrix3DApplied = _loc13_;
            }
            return true;
         }
         if(param2.is3D)
         {
            _loc4_ = new Matrix3D();
            _loc5_ = param2.getValue(param1,Tweenables.ROTATION_X) * Math.PI / 180;
            _loc6_ = param2.getValue(param1,Tweenables.ROTATION_Y) * Math.PI / 180;
            _loc7_ = param2.getValue(param1,Tweenables.ROTATION_CONCAT) * Math.PI / 180;
            _loc4_.prepend(MatrixTransformer3D.rotateAboutAxis(_loc7_,MatrixTransformer3D.AXIS_Z));
            _loc4_.prepend(MatrixTransformer3D.rotateAboutAxis(_loc6_,MatrixTransformer3D.AXIS_Y));
            _loc4_.prepend(MatrixTransformer3D.rotateAboutAxis(_loc5_,MatrixTransformer3D.AXIS_X));
            _loc8_ = param2.getValue(param1,Tweenables.X);
            _loc9_ = param2.getValue(param1,Tweenables.Y);
            _loc10_ = param2.getValue(param1,Tweenables.Z);
            if(getSign(_loc8_) != 0 || getSign(_loc9_) != 0 || getSign(_loc10_) != 0)
            {
               _loc4_.appendTranslation(_loc8_,_loc9_,_loc10_);
            }
            _loc4_.prependTranslation(-this.transformationPoint.x,-this.transformationPoint.y,-this.transformationPointZ);
            if(this._initialPosition)
            {
               _loc4_.appendTranslation(this._initialPosition.x,this._initialPosition.y,this._initialPosition.z);
            }
            _loc11_ = this.getScaleSkewMatrix(param2,param1,this.transformationPoint.x,this.transformationPoint.y);
            _loc12_ = convertMatrixToMatrix3D(_loc11_);
            _loc4_.prepend(_loc12_);
            if(this._initialMatrixOfTarget)
            {
               _loc4_.append(this._initialMatrixOfTarget);
            }
            if(!motionArray || !_lastMatrix3DApplied || !matrices3DEqual(_loc4_,Matrix3D(_lastMatrix3DApplied)))
            {
               this._target.transform.matrix3D = _loc4_;
               _lastMatrix3DApplied = _loc4_;
            }
         }
         return false;
      }
