...
package fl.motion
{
   ...
   public class MatrixTransformer3D
   {
      ...
      public static const AXIS_X:int = // method body index: 311 method index: 311
      0;
      
      public static const AXIS_Y:int = // method body index: 311 method index: 311
      1;
      
      public static const AXIS_Z:int = // method body index: 311 method index: 311
      2;
      ...
      public static function rotateAxis(param1:Matrix3D, param2:Number, param3:int) : Matrix3D
      {
         ...
         var _loc10_:Vector3D = getVector(param1,param3);
         _loc9_.prependRotation(param2 * 180 / Math.PI,_loc10_);
         var _loc11_:int = 0;
         while(_loc11_ < 3)
         {
            if(_loc11_ != param3)
            {
               _loc4_ = getVector(param1,_loc11_);
               _loc5_ = new Array(3);
               _loc6_ = 0;
               while(_loc6_ < 3)
               {
                  _loc8_ = getVector(_loc9_,_loc6_);
                  _loc5_[_loc6_] = _loc4_.dotProduct(_loc8_);
                  _loc6_++;
               }
               _loc4_.x = _loc5_[0];
               _loc4_.y = _loc5_[1];
               _loc4_.z = _loc5_[2];
               _loc4_.w = 0;
               _loc4_ = normalizeVector(_loc4_);
               _loc7_ = Vector.<Number>(getRawDataVector(param1));
               _loc7_[_loc11_ * 4] = _loc4_.x;
               _loc7_[_loc11_ * 4 + 1] = _loc4_.y;
               _loc7_[_loc11_ * 4 + 2] = _loc4_.z;
               _loc7_[_loc11_ * 4 + 3] = _loc4_.w;
               param1 = new Matrix3D(Vector.<Number>(_loc7_));
            }
            _loc11_++;
         }
         return param1;
