...
public class BetterInventory extends Sprite {
    ...
    public static var copy:SimpleButton;
    ...
    public function BetterInventory() {
        ...
        BetterInventory.debug_tf.visible = true;
        BetterInventory.copy.enabled = true;
        BetterInventory.copy.addEventListener(MouseEvent.CLICK, this.onCopy);
        this.filterButton = new BSButtonHintData("$FILTER", "Ctrl", "PSN_L1", "Xenon_L1", 1,
                this.onFilterButtonPress);
        addEventListener(Event.ADDED_TO_STAGE, this.addedToStageHandler);
    }

    private function onCopy(event:Event, text: String):void {
        log2("Trying to fucking copy smth" + text);
        Clipboard.generalClipboard.clear();
        Clipboard.generalClipboard.setData(ClipboardFormats.TEXT_FORMAT, "some_fucking_text" + text,
                false);
    }
    ...
    private function updateFilterButton():void {
        ...
        this.filterButton.ButtonVisible = this.pipboyMenu.DataObj.CurrentPage == INV_PAGE_INDEX
    ...
    private function setSelectedTabLabel(param1:String):void {
        ...
        this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.LeftOne.x = this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.Selected.x
                ...
                - this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.LeftOne.width / 2 - 12.5;
        this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.LeftTwo.x = this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.LeftOne.x
                ...
                - this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.LeftTwo.width / 2 - 12.5;
        this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.RightOne.x = this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.Selected.x
                + this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.Selected.width / 2
                + this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.RightOne.width / 2 + 12.5;
        this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.RightTwo.x = this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.RightOne.x
                ...
                + this.pipboyMenu.Header_mc.TabHeader_mc.AlphaHolder.RightTwo.width / 2 + 12.5;
    }
        ...
        if (this._stackWeightInvalidated) {
            ...
            try {
                ...
                if (selectedEntry && selectedEntry.count > 1) {
                    i = 0;
                    while (true) {
                        if (i < this.invPage.ItemCard_mc.numChildren) {
                            ...
                            if (getQualifiedClassName(itemCard) == "ItemCard_StandardEntry") {
                                if (itemCard.Label_tf.text == this._weightStr) {
                                    actualLabelWidth = itemCard.Label_tf.getLineMetrics(0).width
                                            + 5;
                                    ...
                                    weightSingleDecimalPlaces = itemCard.Value_tf.text.indexOf(".")
                                    > -1 ? int(itemCard.Value_tf.text.length - 1
                                            - itemCard.Value_tf.text.indexOf(".")) : 0;
                                    totalWeight = weightSingle * selectedEntry.count;
                                    itemCard.Value_tf.text = weightSingle + " ["
                                            + totalWeight.toFixed(weightSingleDecimalPlaces) + "]";
                                    break;
                                }
                            }
                            i++;
    ...
    public static function log2(...rest):void {
        // method body index: 241 method index: 241
        if (!DEBUG_MODE) {
            return;
        }
        debug_tf.textColor = 0xff0000;
        debug_tf.appendText(rest[0]);
    }
